...
static struct gpio_key* gpio_keys_maple; //设定自己的按键中断对象类型的指针
...
static int keys_probe(struct platform_device *pdev)
{
    ...
    /* 1. 从设备树上获取特定compatible-ID的硬件gpio数量 */
    count = of_gpio_count(node);
    if (!count) {
        dev_err(dev, "设备树中缺失有效 GPIO\n");
        return -EINVAL;
    }

    /* 2. 申请自定义驱动对象(struct gpio_key类型)所需的存储空间 */
    gpio_keys_maple = devm_kzalloc(dev, sizeof(struct gpio_key)*count, GFP_KERNEL);
	...
	*/
    for(i =0; i < count; i++){ 
		/* 3.1 解析出GPIO的引脚的标志（flag）和引脚编号 */
        gpio_keys_maple[i].gpio  = of_get_gpio_flags(node, i, &flag);      
		...
		*/
        gpio_keys_maple[i].gpiod = devm_gpiod_get_index(dev, NULL, i, GPIOD_IN); 
        if (IS_ERR(gpio_keys_maple[i].gpiod)) {
            return PTR_ERR(gpio_keys_maple[i].gpiod);
        }
		
        gpio_keys_maple[i].flag  = flag & OF_GPIO_ACTIVE_LOW;

		/* 3.3 根据GPIO编号，转换出对应的中断号 */
        gpio_keys_maple[i].irq = gpiod_to_irq(gpio_keys_maple[i].gpiod);   
        if (gpio_keys_maple[i].irq < 0) {
            return gpio_keys_maple[i].irq;
        }
		
		/* 3.4 注册GPIO引脚的中断资源 */
		ret = devm_request_irq(dev, 
						gpio_keys_maple[i].irq, 
						gpio_key_isr, 
